...
export default function WebhooksPage() {
  ...
  const fetchWebhooks = async () => {
    setIsLoading(true);
    try {
      const epRes = await tarakFetch("/apis/webhook.tarak.io/v1/endpoints");
      if (epRes.data?.items) {
        setWebhooks(epRes.data.items);
      }
      const delRes = await tarakFetch("/apis/webhook.tarak.io/v1/deliveries");
      if (delRes.data?.items) {
        setDeliveries(delRes.data.items);
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {webhooks.length === 0 ? (
        <div className="p-12 rounded-2xl bg-slate-900/40 border border-white/10 text-center space-y-3">
          <Zap size={32} className="text-slate-600 mx-auto" />
          <h3 className="text-sm font-bold text-white">No Webhooks Registered</h3>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Configure webhook endpoints to receive instant alerts for pod crashes, node failovers, policy violations, and scaling events.
          </p>
        </div>
      ) : (
      ...
      <div className="space-y-3">
        ...
        {deliveries.length === 0 ? (
          <div className="p-8 rounded-2xl bg-slate-900/40 border border-white/10 text-center text-xs text-slate-500 font-mono">
            No webhook delivery history recorded yet. Deliveries will appear here as events trigger.
          </div>
        ) : (
